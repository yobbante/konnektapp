/**
 * AerienMissionsTab - Mission marketplace for air cargo transporters
 * Shows freight_requests with freight_mode='air'
 */
import { useState, useEffect, useCallback } from "react";
import { motion, AnimatePresence } from "framer-motion";
import {
  Plane, Scale, Clock, MapPin, RefreshCw, Zap,
...
import { AerienNegotiationSheet } from "./AerienNegotiationSheet";
import { format, formatDistanceToNow } from "date-fns";
import { fr } from "date-fns/locale";
import { cn } from "@/lib/utils";

interface AerienMissionsTabProps {
  gpId: string;
}
...
const urgencyConfig: Record<string, { label: string; color: string; bg: string }> = {
  ...
  critical: { label: "Critique", color: "text-red-700", bg: "bg-red-100 dark:bg-red-900/30" },
};

export function AerienMissionsTab({ gpId }: AerienMissionsTabProps) {
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-border bg-card p-3 space-y-2">
        <div className="flex items-center gap-2">
          <div className="flex-1 relative">
            <Plane className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-violet-500" />
            <Input placeholder="Aéroport départ" value={searchOrigin} onChange={e => setSearchOrigin(e.target.value)} className="h-9 pl-8 text-sm" />
          </div>
          <ArrowRight className="w-4 h-4 text-muted-foreground shrink-0" />
          <div className="flex-1 relative">
            <Plane className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-primary" />
            <Input placeholder="Aéroport arrivée" value={searchDest} onChange={e => setSearchDest(e.target.value)} className="h-9 pl-8 text-sm" />
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">
          {filteredMissions.length} demande{filteredMissions.length !== 1 ? "s" : ""} cargo
        </p>
      ...
      {filteredMissions.length === 0 ? (
        <Card className="border-dashed">
          <CardContent className="py-12 text-center">
            ...
            <h3 className="font-semibold mb-1 text-sm">Aucune demande cargo</h3>
            <p className="text-xs text-muted-foreground">
        ...
        <div className="space-y-2">
          <AnimatePresence>
            {filteredMissions.map((m, idx) => {
              ...
              return (
                <motion.div key={m.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -50 }} transition={{ delay: idx * 0.03 }}>
                  <Card className={cn("overflow-hidden cursor-pointer transition-all hover:shadow-md", isExpanded && "ring-2 ring-primary shadow-lg")}
                    ...
                    <CardContent className="p-0">
                      <div className="p-3">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 mb-1">
                              ...
                              <span className="text-sm font-semibold truncate">{m.origin_port_or_airport || m.origin_city}</span>
                            </div>
                            <div className="flex items-center gap-1.5">
                              <div className="w-2 h-2 rounded-full bg-primary shrink-0" />
                              <span className="text-sm font-semibold truncate">{m.destination_port_or_airport || m.destination_city}</span>
                            </div>
                          </div>
                          {budget > 0 && (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1.5">
                            <Badge className={cn("text-[9px] h-4 px-1.5 font-semibold", cType.bg, cType.color)}>
                              {cType.label}
                            </Badge>
                            ...
                            {m.weight_kg && (
                              <Badge variant="outline" className="text-[10px] h-5 px-2 font-bold">{m.weight_kg} kg</Badge>
                            )}
                          </div>
                        </div>
                      </div>
                            ...
                            <div className="px-3 pb-3 pt-2 border-t bg-muted/30 space-y-3">
                              <div className="grid grid-cols-3 gap-2">
                                ...
                                <div className="text-center p-2 rounded-lg bg-background">
                                  ...
                                  <p className="text-xs font-bold">{m.dimensions_cm || "—"}</p>
                                  <p className="text-[9px] text-muted-foreground">Dimensions</p>
                                </div>
                                <div className="text-center p-2 rounded-lg bg-background">
                                  <Plane className="w-3.5 h-3.5 mx-auto text-muted-foreground mb-0.5" />
                                  <p className="text-xs font-bold">{cType.label}</p>
                                  <p className="text-[9px] text-muted-foreground">Type</p>
                                </div>
                              </div>

                              {m.merchandise_description && (
                                <div className="p-2.5 bg-background rounded-lg text-xs text-muted-foreground">{m.merchandise_description}</div>
                              )}
                              ...
                              <div className="flex flex-wrap gap-1.5">
                                ...
                                {m.is_urgent && <Badge variant="secondary" className="text-[10px]">Urgent</Badge>}
                              </div>

                              <Button className="w-full" onClick={(e) => { e.stopPropagation(); openNegotiation(m); }}>
                                <MessageCircle className="w-4 h-4 mr-2" /> Proposer un prix
                              </Button>
                            </div>
                          </motion.div>
                        )}
