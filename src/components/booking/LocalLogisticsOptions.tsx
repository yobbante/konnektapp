...
export function LocalLogisticsOptions({
  ...
  // V1.2: Prices in FCFA
  const pickupPrice = useMemo(() => pickupEnabled ? calculatePrice(pricingConfig.pickup) : 0, [pickupEnabled, weight, isFragile, pricingConfig.pickup]);
  const deliveryPrice = useMemo(() => deliveryEnabled ? calculatePrice(pricingConfig.delivery) : 0, [deliveryEnabled, weight, isFragile, pricingConfig.delivery]);
  ...
  }, [pickupEnabled, deliveryEnabled, pickupAddress, pickupContactName, pickupPhone, pickupWhatsapp, pickupTimeSlot, deliveryAddress, deliveryContactName, deliveryPhone, deliveryWhatsapp, deliveryInstructions, pickupPrice, deliveryPrice, totalLogisticsPrice, termsAccepted]);
      ...
      <Card className="border-2 border-dashed border-primary/30 bg-gradient-to-br from-primary/5 to-transparent">
        ...
        <CardContent className="space-y-4">
          ...
          {originSupported && <div className="space-y-3">
              <label className="flex items-start gap-3 p-3 bg-muted/30 rounded-xl cursor-pointer hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {pricingConfig.pickup && <div className="ml-auto flex flex-col items-end">
                        <Badge className="bg-primary/10 text-primary hover:bg-primary/20">
                          {pickupPrice.toLocaleString()} FCFA
                        </Badge>
                        {!isFCFA && <span className="text-[10px] text-muted-foreground mt-0.5">
              ...
              {/* Pickup Fields */}
              <AnimatePresence>
                {pickupEnabled && <motion.div initial={{
              opacity: 0,
              height: 0
            }} animate={{
              opacity: 1,
          ...
          {destinationSupported && <div className="space-y-3">
              <label className="flex items-start gap-3 p-3 bg-muted/30 rounded-xl cursor-pointer hover:bg-muted/50 transition-colors">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {pricingConfig.delivery && <div className="ml-auto flex flex-col items-end">
              ...
              {/* Delivery Fields */}
              <AnimatePresence>
                {deliveryEnabled && <motion.div initial={{
              opacity: 0,
              height: 0
            }} animate={{
              opacity: 1,
          ...
          {!destinationSupported && <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200">
          ...
          {/* Price Summary */}
          {(pickupEnabled || deliveryEnabled) && <motion.div initial={{
          opacity: 0,
          y: 10
        }} animate={{
          opacity: 1,
          y: 0
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <div className="text-right">
                  ...
                  {!isFCFA && <span className="text-xs text-muted-foreground">
          ...
          <div className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-200">
            ...
            <p className="text-xs text-blue-700 dark:text-blue-400">Cette option facilite la remise et la réception du colis. Service disponible uniquement à Dakar.</p>
          </div>
        </CardContent>
      </Card>
    </div>;
