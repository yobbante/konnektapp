...
export function AerienDashboardLayout({
  ...
  const getActiveTab = () => {
    ...
    if (path.includes("/aerien/demandes") || path.includes("/aerien/en-cours")) return "missions";
    if (path.includes("/aerien/messages")) return "messages";
    if (path.includes("/aerien/profil-public") || path.includes("/aerien/parametres") || path.includes("/aerien/historique") || path.includes("/aerien/wallet")) return "profil";
    return "apercu";
  };
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <GPNotificationsDropdown gpProfileId={gpProfile.id} isOpen={showNotifications} onClose={() => setShowNotifications(false)} onViewOrderDetail={(orderId) => navigate(`/aerien/order/${orderId}`)} />
...
function NavItem({ icon: Icon, label, active, badge, locked, onClick }: {
  ...
  return (
    <button onClick={onClick} disabled={locked}
      ...
      {active && (
        <motion.div layoutId="aerien-nav" className="absolute bottom-1 w-1 h-1 rounded-full bg-primary" transition={{ type: "spring", stiffness: 500, damping: 30 }} />
      )}
    </button>
  );
}
