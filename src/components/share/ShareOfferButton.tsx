...
export function ShareOfferButton({ 
  ...
  const shareText = `🚀 Offre de transport ${originCity} → ${destinationCity} le ${dateFormatted} - ${pricePerKg} ${getCurrencySymbol(currency)}/kg sur Yobbanté`;
  ...
  const handleCopyLink = async () => {
    try {
      ...
      toast({
        title: "Lien copié !",
        description: "Le lien de l'offre a été copié",
      });
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Transport ${originCity} → ${destinationCity}`,
          text: shareText,
          url: offerUrl,
        });
        setOpen(false);
  ...
  const shareLinks = [
    {
      name: "WhatsApp",
      icon: MessageCircle,
      url: `https://wa.me/?text=${encodeURIComponent(shareText + " " + offerUrl)}`,
      color: "bg-green-500 hover:bg-green-600",
    },
    {
      name: "Telegram",
      icon: Send,
      url: `https://t.me/share/url?url=${encodeURIComponent(offerUrl)}&text=${encodeURIComponent(shareText)}`,
      color: "bg-blue-500 hover:bg-blue-600",
    },
    {
      name: "Facebook",
      icon: Share2,
      url: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(offerUrl)}`,
      color: "bg-blue-600 hover:bg-blue-700",
    },
    {
      name: "Twitter",
      icon: Share2,
      url: `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(offerUrl)}`,
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-72 p-3" align="end">
        <div className="space-y-3">
          <h4 className="font-semibold text-sm">Partager cette offre</h4>
