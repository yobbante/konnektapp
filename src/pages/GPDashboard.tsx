...
import { Package, Wallet, Plus, ChevronRight, Star, TrendingUp, Clock, MapPin, ArrowRight, LogOut, AlertTriangle, CheckCircle, Truck, ChevronDown, ChevronUp, ArrowLeft, BarChart3, User, ShieldAlert, EyeOff, Bell, Copy, FileText } from "lucide-react";
...
import { OrderStatus, orderStatusConfig, getOrderStatusLabel, getOrderStatusColor, getNextOrderStatus } from "@/lib/transportTypes";
...
export default function GPDashboard() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('gp-orders-realtime').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'orders',
  ...
  const checkAuthAndLoadData = async () => {
    try {
      ...
      const {
        data: profile,
        error: profileError
      } = await supabase.from("gp_profiles").select("*").eq("user_id", user.id).maybeSingle();
      ...
      const {
        data: walletData
      } = await supabase.from("gp_wallets").select("*").eq("gp_id", profile.id).maybeSingle();
      setWallet(walletData);

      // Filtrer uniquement les offres actives et valides
      const {
        data: offersData
      } = await supabase.from("gp_offers").select("*").eq("gp_id", profile.id).eq("status", "active").gte("departure_date", new Date().toISOString()).order("created_at", {
        ascending: false
      });
      setOffers(offersData || []);
      const {
        data: ordersData
      } = await supabase.from("orders").select("*").eq("gp_id", profile.id).order("created_at", {
        ascending: false
      });
  ...
  // Calculate stats
  const pendingUpdateOrders = orders.filter(o => ['accepted', 'collected', 'in_transit'].includes(o.status));
  ...
  const activeOrder = orders.find(o => ['accepted', 'collected', 'in_transit'].includes(o.status));
  return <div className="min-h-screen pb-safe bg-background">
      {/* Role-specific Fixed Header with Dropdown - NO Global Header */}
      <div className={`sticky top-0 z-50 ${theme.headerBgClass} ${theme.headerTextClass} py-3 px-4 shadow-md`} style={{
      paddingTop: 'calc(12px + var(--safe-top, 0px))'
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <GPDropdownMenu activeTab={activeTab} onTabChange={setActiveTab} onCreateOffer={() => setShowCreateOffer(true)} />
          </div>
        </div>
      </div>
      ...
      {isPendingValidation && <div className="px-4 pt-4">
      ...
      {(isRejected || isSuspended) && <div className="px-4 pt-4">
            ...
            <CardContent className="py-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    {isRejected ? "Votre demande a été refusée. Contactez le support pour plus d'informations." : "Votre compte a été suspendu. Contactez le support pour résoudre cette situation."}
                  </p>
                </div>
              </div>
            </CardContent>
      ...
      {activeOrder && !isPendingValidation && <ActiveOrderBar order={activeOrder} onRefresh={checkAuthAndLoadData} />}

      {/* Rappel pour les missions en cours */}
      {pendingUpdateOrders.length > 0 && activeTab === "overview" && !isPendingValidation && <div className="px-4 pt-4">
          <motion.div initial={{
        opacity: 0,
        y: -10
      }} animate={{
        ...
        y: 0
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <Button variant="outline" size="sm" className="mt-2" onClick={() => setActiveTab("orders")}>
                  Voir les missions
      ...
      {activeTab === "overview" && <ModernOverviewTab gpProfile={gpProfile} kpiStats={kpiStats} stats={stats} wallet={wallet} offers={offers} orders={orders} pendingOrders={stats.pendingOrders} onCreateOffer={() => setShowCreateOffer(true)} onSignOut={handleSignOut} onViewOrders={() => setActiveTab("orders")} onViewWallet={() => setActiveTab("wallet")} onViewProfile={() => navigate("/transporter/profile")} onRefresh={checkAuthAndLoadData} />}

      {activeTab === "offers" && <OffersTab offers={offers} onCreateOffer={() => setShowCreateOffer(true)} onRefresh={checkAuthAndLoadData} onBack={handleBackToOverview} />}

      {activeTab === "orders" && <OrdersTab orders={orders} gpProfileId={gpProfile.id} onRefresh={checkAuthAndLoadData} onBack={handleBackToOverview} />}

      {activeTab === "wallet" && <WalletTab wallet={wallet} onBack={handleBackToOverview} />}

      {activeTab === "stats" && <StatsTab orders={orders} gpProfileId={gpProfile.id} onBack={handleBackToOverview} />}

      {activeTab === "profile" && <div className="px-4 py-4">
          <Button variant="ghost" size="sm" onClick={handleBackToOverview} className="-ml-2 mb-4">
            <ArrowLeft className="w-4 h-4 mr-1" />
            Retour
          </Button>
          <Button variant="default" className="w-full" onClick={() => navigate("/transporter/profile")}>
      ...
      <GPNotificationsDropdown gpProfileId={gpProfile.id} isOpen={showNotifications} onClose={() => setShowNotifications(false)} onViewOrderDetail={orderId => navigate(`/gp/order/${orderId}`)} />

      {activeTab === "requests" && <div className="px-4 py-4">
          <Button variant="ghost" size="sm" onClick={handleBackToOverview} className="-ml-2 mb-4">
            <ArrowLeft className="w-4 h-4 mr-1" />
            Retour
          </Button>
          <Button variant="default" className="w-full" onClick={() => navigate("/gp/demandes")}>
      ...
      {gpProfile.gp_type === "bagages_international" && activeTab === "overview" && !isPendingValidation && <AddDepartureFAB onClick={() => setShowCreateBaggageVoyage(true)} />}

      <GPMobileNav activeTab={activeTab} onTabChange={setActiveTab} />

      {/* Create Offer Dialog for non-baggage GPs */}
      <GPCreateOfferDialog open={showCreateOffer} onClose={() => setShowCreateOffer(false)} gpProfile={gpProfile} onSuccess={() => {
      setShowCreateOffer(false);
      checkAuthAndLoadData();
    }} />

      {/* Create Baggage Voyage Dialog for baggage GPs */}
      <CreateBaggageVoyageDialog open={showCreateBaggageVoyage} onClose={() => setShowCreateBaggageVoyage(false)} gpId={gpProfile.id} lastVoyage={offers.length > 0 ? offers[0] : null} onSuccess={() => {
...
function ModernOverviewTab({
  ...
  useEffect(() => {
    if (showMoreOptions && moreOptionsRef.current) {
      setTimeout(() => {
        moreOptionsRef.current?.scrollIntoView({
          behavior: "smooth",
          block: "start"
        });
      }, 100);
  ...
  return <div className="px-4 py-4 space-y-4">
      ...
      {isBagagesGP && <BagagesDashboardSection gpId={gpProfile.id} gpProfile={gpProfile} voyages={offers} orders={orders} onRefresh={onRefresh} onCreateVoyage={onCreateOffer} />}

      {/* PRIORITY 1: Create Offer CTA - EN HAUT (only for non-bagages GPs) */}
      {!isBagagesGP && <motion.div initial={{
      opacity: 0,
      y: 10
    }} animate={{
      opacity: 1,
      ...
      <RecentHistory orders={orders} onViewAll={onViewOrders} onRefresh={onRefresh} />

      {/* Smart Notifications */}
      <SmartNotifications pendingMissions={pendingOrders} lastPayment={wallet?.total_earned > 0 ? {
      amount: wallet.balance,
      date: "Récemment"
    } : undefined} highDemandZone={gpProfile.zones_covered?.[0]} />
      ...
      <Collapsible open={showMoreOptions} onOpenChange={setShowMoreOptions}>
        ...
        <CollapsibleContent className="pt-3 space-y-3">
          {/* Quick Actions */}
          <QuickActions onUpdateProfile={onViewProfile} onViewMissions={onViewOrders} onViewHistory={onViewOrders} onViewStats={onViewWallet} />

          {/* Badge System */}
          <BadgeSystem isVerified={gpProfile.status === 'verified'} rating={gpProfile.rating || 0} totalDeliveries={gpProfile.total_deliveries || 0} totalVolume={kpiStats.totalVolume} isPremium={gpProfile.subscription === 'premium'} gpType={gpProfile.gp_type} />
        </CollapsibleContent>
      </Collapsible>
      </div>

      {/* Departure Calendar - Integrated (only for non-bagages GPs) */}
      {!isBagagesGP && <DepartureCalendar gpId={gpProfile.id} onCreateOffer={onCreateOffer} />}
  ...
  return <>
      ...
      y: 0
        ...
        <div className="flex gap-2 mt-3 pt-3 border-t border-border">
          <Button variant="outline" size="sm" className="flex-1 h-8 text-xs" onClick={toggleOfferStatus} disabled={updating}>
            {updating ? <div className="w-3 h-3 rounded-full border-2 border-current border-t-transparent animate-spin" /> : offer.status === 'active' ? <>
          ...
          <Button variant="secondary" size="sm" className="h-8 text-xs px-2" onClick={() => setShowEditDialog(true)}>
            <Package className="w-3 h-3" />
          </Button>
          <Button variant="outline" size="sm" className="h-8 text-xs px-2" onClick={() => onDuplicate(offer)} title="Dupliquer cette offre">
            <Copy className="w-3 h-3" />
          </Button>
          <Button variant="default" size="sm" className="h-8 text-xs px-2" onClick={() => navigate(`/offres/${offer.id}`)}>
      ...
      {/* Edit Dialog */}
      <GPEditOfferDialog open={showEditDialog} onClose={() => setShowEditDialog(false)} offer={offer} onSuccess={() => {
...
function OffersTab({
  ...
  const filteredOffers = offers.filter((offer: any) => {
    // Search filter
    const searchMatch = filters.search === "" || offer.origin_city?.toLowerCase().includes(filters.search.toLowerCase()) || offer.destination_city?.toLowerCase().includes(filters.search.toLowerCase());

    // Status filter
    const statusMatch = filters.status === "all" || offer.status === filters.status;

    // City filter
    const cityMatch = filters.city === "" || offer.origin_city?.toLowerCase().includes(filters.city.toLowerCase()) || offer.destination_city?.toLowerCase().includes(filters.city.toLowerCase());
  ...
  const handleDuplicateOffer = async (offer: any) => {
    ...
    try {
      ...
      const {
        data: gpProfile
      } = await supabase.from("gp_profiles").select("id").eq("user_id", user.id).single();
      ...
      const {
        ...
        available_capacity: offer.total_capacity,
  ...
  return <div className="px-4 py-4 space-y-4">
      ...
      {/* Filters */}
      <OfferFilters activeFilters={filters} onFilterChange={setFilters} />
      ...
      {filteredOffers.length === 0 ? <div className="mobile-card text-center py-8">
          <Package className="w-12 h-12 text-muted-foreground/50 mx-auto mb-3" />
          <p className="text-muted-foreground mb-4">
            {offers.length === 0 ? "Aucune offre active" : "Aucune offre ne correspond aux filtres"}
          </p>
          {offers.length === 0 && <Button variant="default" onClick={onCreateOffer}>Créer une offre</Button>}
        </div> : <div className="space-y-3">
          {filteredOffers.map((offer: any) => <OfferCard key={offer.id} offer={offer} onRefresh={onRefresh} onDuplicate={handleDuplicateOffer} />)}
        </div>}
    </div>;
}
...
function OrdersTab({
  ...
  const OrderCard = ({
    ...
    return <div className="mobile-card">
        ...
        {/* Workflow buttons */}
        {nextStatus && nextLabel && <Button variant={nextStatus === 'delivered' ? 'success' : nextStatus === 'accepted' ? 'success' : 'secondary'} size="sm" className="w-full" disabled={isUpdating} onClick={() => updateOrderStatus(order.id, nextStatus)}>
            {isUpdating ? <div className="w-4 h-4 rounded-full border-2 border-current border-t-transparent animate-spin" /> : <>
        ...
        {status === 'delivered' && <div className="flex items-center justify-center gap-2 text-success text-sm">
  ...
  return <div className="px-4 py-4 space-y-6">
      ...
      {orders.length === 0 ? <div className="mobile-card text-center py-8">
          <Package className="w-12 h-12 text-muted-foreground/50 mx-auto mb-3" />
...
function WalletTab({
  ...
  return <div className="px-4 py-4 space-y-4">
      ...
      y: 0
        ...
        {(wallet?.pending_balance || 0) > 0 && <div className="mt-3 pt-3 border-t border-primary-foreground/20">
