...
import { Package, Truck, ArrowRight, Zap, Ship, Plane, MapPin, Star, Shield, Clock, Briefcase, Building2, Scale, Weight, Download } from "lucide-react";
...
const transportTypes = [{
  type: "voyageur" as TransportType,
  icon: Briefcase,
  label: "GP",
  color: "bg-transport-voyageur/10 text-transport-voyageur border-transport-voyageur/20"
}, {
  type: "agence" as TransportType,
  icon: Building2,
  label: "Agence",
  color: "bg-transport-agence/10 text-transport-agence border-transport-agence/20"
}, {
  type: "express" as TransportType,
  icon: Zap,
  label: "Express",
  color: "bg-transport-express/10 text-transport-express border-transport-express/20"
}, {
  type: "routier" as TransportType,
  icon: Truck,
  label: "Routier",
  color: "bg-transport-routier/10 text-transport-routier border-transport-routier/20"
}, {
  type: "maritime" as TransportType,
  icon: Ship,
  label: "Maritime",
  color: "bg-transport-maritime/10 text-transport-maritime border-transport-maritime/20"
}, {
  type: "aerien" as TransportType,
  icon: Plane,
  label: "Aérien",
  color: "bg-transport-aerien/10 text-transport-aerien border-transport-aerien/20"
}];
  ...
  const loadOffers = async () => {
    try {
      ...
      const {
        data: offersData,
        error: offersError
      } = await supabase.from("gp_offers").select(`
          *,
          vehicles(id, name, vehicle_type, max_weight_kg)
        `).eq("status", "active").gte("departure_date", new Date().toISOString()).order("departure_date", {
        ascending: true
      }).limit(6);
      ...
      if (offersData && offersData.length > 0) {
        ...
        const {
          data: profiles
        } = await supabase.from("public_gp_profiles").select("id, business_name, rating").in("id", gpIds);
        const profilesMap = new Map(profiles?.map(p => [p.id, p]) || []);
        const mappedData = offersData.map(offer => ({
          ...offer,
          gp_profiles: profilesMap.get(offer.gp_id) || {
            business_name: "Transporteur",
            rating: 0
          }
        }));
      ...
      <section className="px-4 py-6">
        ...
        y: 0
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-5">
            ...
            {isGP ? <Link to="/gp/dashboard" className="flex-1">
                <Button variant="outline" size="lg" className="w-full">
                  <Truck className="w-5 h-5" />
                  Mon Dashboard GP
                </Button>
              </Link> : <Link to="/gp/inscription" className="flex-1">
                <Button variant="outline" size="lg" className="w-full">
                  <Truck className="w-5 h-5" />
      ...
      <section className="px-4 py-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            {selectedType && <button onClick={() => setSelectedType(null)} className="text-xs text-primary font-medium hover:underline">
                Tout voir
              </button>}
            {/* Advanced Filters */}
            <HomeAdvancedFilters filters={advancedFilters} onFiltersChange={setAdvancedFilters} />
            ...
            <Sheet open={showFilterSheet} onOpenChange={setShowFilterSheet}>
              ...
              <SheetContent side="bottom" className="h-[50vh] rounded-t-2xl">
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  {transportTypes.map(type => <button key={type.label} onClick={() => {
                  setSelectedType(selectedType === type.type ? null : type.type);
                  setShowFilterSheet(false);
                }} className={`flex items-center gap-3 p-4 rounded-xl border-2 transition-all ${selectedType === type.type ? "bg-primary text-primary-foreground border-primary" : `${type.color} hover:shadow-md bg-card`}`}>
        ...
        <div className="flex flex-col lg:flex-row lg:gap-6">
          ...
          <div className="lg:w-48 lg:flex-shrink-0 mb-4 lg:mb-0">
            ...
            <div className="flex lg:hidden gap-2 overflow-x-auto pb-2 -mx-1 px-1">
              {transportTypes.map((type, index) => <motion.button key={type.label} initial={{
              opacity: 0,
              scale: 0.9
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.03
            }} onClick={() => setSelectedType(selectedType === type.type ? null : type.type)} className={`flex items-center justify-center w-10 h-10 rounded-xl border-2 transition-all flex-shrink-0 ${selectedType === type.type ? "bg-primary text-primary-foreground border-primary shadow-lg" : `${type.color} hover:shadow-md bg-card`}`} title={type.label}>
                  <type.icon className={`w-5 h-5 ${selectedType === type.type ? 'text-primary-foreground' : ''}`} />
                </motion.button>)}
            </div>
            ...
            <div className="hidden lg:flex lg:flex-col gap-2">
              ...
              delay: index * 0.03
            }} onClick={() => setSelectedType(selectedType === type.type ? null : type.type)} className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all w-full ${selectedType === type.type ? "bg-primary text-primary-foreground border-primary shadow-lg" : `${type.color} hover:shadow-md bg-card`}`}>
