...
import { 
  Truck, Package, MapPin, Calendar, Clock, Home, Building2,
  ChevronRight, ChevronLeft, Check, ArrowRight, Plus, Minus,
  Shield, Star, Info, Camera, FileText, Phone, User, Calculator
} from "lucide-react";
...
const defaultItems: RoomItem[] = [
  { id: "bed", name: "Lit", quantity: 0, icon: "🛏️" },
  { id: "sofa", name: "Canapé", quantity: 0, icon: "🛋️" },
  { id: "table", name: "Table", quantity: 0, icon: "🪑" },
  { id: "chair", name: "Chaise", quantity: 0, icon: "💺" },
  { id: "wardrobe", name: "Armoire", quantity: 0, icon: "🚪" },
  { id: "fridge", name: "Réfrigérateur", quantity: 0, icon: "❄️" },
  { id: "washer", name: "Machine à laver", quantity: 0, icon: "🧺" },
  { id: "tv", name: "TV", quantity: 0, icon: "📺" },
  { id: "boxes", name: "Cartons (estimés)", quantity: 0, icon: "📦" },
];

const timeSlots = [
  { id: "morning", label: "Matin", time: "8h - 12h" },
  { id: "afternoon", label: "Après-midi", time: "14h - 18h" },
  { id: "full", label: "Journée", time: "8h - 18h" },
];

const housingTypes = [
  { id: "studio", label: "Studio", icon: Home, rooms: 1 },
  { id: "apartment", label: "Appartement", icon: Building2, rooms: 3 },
  { id: "house", label: "Maison", icon: Home, rooms: 5 },
  { id: "office", label: "Bureau", icon: Building2, rooms: 2 },
];
...
${formData.additionalNotes ? `Notes: ${formData.additionalNotes}` : ''}`,
        ...
        additional_services: [
          formData.needPacking && "Emballage",
          formData.needUnpacking && "Déballage",
          formData.needFurnitureAssembly && "Montage meubles",
          formData.needStorage && `Stockage ${formData.storageDuration}j`,
                ...
                <div className="grid grid-cols-2 gap-3">
                  {housingTypes.map((type) => {
                    ...
                    return (
                      <button
                        key={type.id}
                        onClick={() => setFormData({ ...formData, housingType: type.id as any, rooms: type.rooms })}
                    ...
                    <div className="flex gap-3">
                      <div className="flex-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => setFormData({ ...formData, originFloor: parseInt(e.target.value) || 0 })}
                        />
                      </div>
                      <div className="flex items-center gap-2 pt-5">
                        <Switch
                          checked={formData.originElevator}
                          onCheckedChange={(checked) => setFormData({ ...formData, originElevator: checked })}
                        />
                    ...
                    <div className="flex gap-3">
                      <div className="flex-1">
                        ...
                        <Input
                          ...
                          onChange={(e) => setFormData({ ...formData, destinationFloor: parseInt(e.target.value) || 0 })}
                        />
                      </div>
                      <div className="flex items-center gap-2 pt-5">
                        <Switch
                          checked={formData.destinationElevator}
                          onCheckedChange={(checked) => setFormData({ ...formData, destinationElevator: checked })}
                        />
                        <Label className="text-xs">Ascenseur</Label>
                      </div>
                    </div>
            ...
            {step === 4 && (
              ...
              >
                ...
                <div>
                  ...
                  <Textarea
                    placeholder="Objets fragiles, accès difficile, etc."
            ...
            {step === 5 && (
              ...
              >
                ...
                <div className="bg-card rounded-2xl border border-border p-4 space-y-3">
                  ...
                  {[
                    { key: "needPacking", label: "Emballage", desc: "On emballe vos affaires", price: "+30K" },
                    { key: "needUnpacking", label: "Déballage", desc: "On déballe à l'arrivée", price: "+25K" },
                    { key: "needFurnitureAssembly", label: "Montage meubles", desc: "Démontage et remontage", price: "+20K" },
                    { key: "needStorage", label: "Stockage temporaire", desc: "Garde-meuble sécurisé", price: "+50K/j" },
                  ].map((service) => (
                    <div key={service.key} className="flex items-center justify-between py-2">
                      <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Switch
                          checked={formData[service.key as keyof MovingFormData] as boolean}
                ...
                <Button 
                  ...
                  disabled={submitting || !formData.contactName || !formData.contactPhone}
                >
                  {submitting ? (
                    <MiniLoader size="sm" />
                  ) : (
