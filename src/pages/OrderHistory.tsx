...
import { 
  Package, Clock, MapPin, ArrowRight, Truck, User, 
  CheckCircle, AlertTriangle, X, Calendar, Scale,
  Phone, MessageCircle, ChevronDown, ExternalLink,
  Star
} from "lucide-react";
...
export default function OrderHistory() {
  ...
  const getStatusInfo = (status: string) => {
    ...
    const configs: Record<string, { color: string; icon: any; bgColor: string }> = {
      pending: { color: "text-amber-600", icon: Clock, bgColor: "bg-amber-500/10" },
      accepted: { color: "text-blue-600", icon: User, bgColor: "bg-blue-500/10" },
      collected: { color: "text-purple-600", icon: Package, bgColor: "bg-purple-500/10" },
      in_transit: { color: "text-secondary", icon: Truck, bgColor: "bg-secondary/10" },
      delivered: { color: "text-success", icon: CheckCircle, bgColor: "bg-success/10" },
      cancelled: { color: "text-destructive", icon: X, bgColor: "bg-destructive/10" },
      disputed: { color: "text-destructive", icon: AlertTriangle, bgColor: "bg-destructive/10" },
    };
    
    return { label, ...configs[validStatus] };
  };
  ...
  return (
    ...
    >
      ...
      {ratingOrder && (
        <RateOrderDialog
          open={!!ratingOrder}
          onOpenChange={() => setRatingOrder(null)}
...
function OrderCard({ order, expanded, onToggle, onRate, navigate }: OrderCardProps) {
  ...
  return (
    <motion.div layout className="overflow-hidden">
      <Card 
        className={`overflow-hidden transition-all ${expanded ? 'ring-2 ring-primary/20' : ''}`}
      >
        {/* Main clickable area */}
        <motion.div 
          whileTap={{ scale: 0.99 }}
