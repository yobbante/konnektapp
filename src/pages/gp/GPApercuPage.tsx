 ...
 * 
 * V1 TERRAIN: 4 status cards (À collecter, En transit, Arrivés, À remettre)
 * + Alerts (weight pending, custom requests)
 * + Upcoming departures
 */
...
interface StatusCounts {
  toCollect: number;    // accepted (waiting for deposit)
  inTransit: number;    // collected + in_transit
  arrived: number;      // arrived (at destination)
  toDeliver: number;    // arrived, ready for handover
}
...
export default function GPApercuPage() {
  ...
  const loadData = async (refresh = false) => {
    ...
    try {
      const [ordersRes, offersRes, customReqRes] = await Promise.all([
        ...
        {!isPending && (
          <div className="grid grid-cols-2 gap-3">
            <StatusCard
              ...
              color="amber"
              onClick={() => navigate("/gp/colis?filter=accepted")}
            />
            <StatusCard
              icon={Plane}
              label="En transit"
              ...
              color="blue"
              onClick={() => navigate("/gp/colis?filter=in_transit")}
            />
            <StatusCard
              icon={MapPin}
              label="Arrivés"
              ...
              color="purple"
              onClick={() => navigate("/gp/colis?filter=arrived")}
            />
            <StatusCard
              icon={Send}
              label="À remettre"
              count={counts.toDeliver}
              color="green"
              onClick={() => navigate("/gp/distribution")}
            />
          </div>
        )}
          ...
          <div className="space-y-2">
            ...
            {customRequests > 0 && (
              <AlertBanner
                ...
                title={`${customRequests} demande${customRequests > 1 ? "s" : ""} personnalisée${customRequests > 1 ? "s" : ""}`}
            ...
            {upcomingDepartures.length === 0 ? (
              <Card className="border-dashed">
                <CardContent className="py-8 text-center text-muted-foreground">
                  ...
                  <Button variant="link" size="sm" className="mt-1" onClick={() => navigate("/gp/calendrier")}>
                    Ajouter un voyage
                  </Button>
                </CardContent>
              </Card>
