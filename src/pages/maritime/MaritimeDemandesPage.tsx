/**
 * MaritimeDemandesPage — Full maritime mission hub for transitaires
 * Tabs: Demandes (orders) | Missions (marketplace) | Consolidation | Mes offres
 */
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import { Ship, Package, RefreshCw, Check, X, Bell, ChevronRight, Users, Zap, Anchor } from "lucide-react";
...
import { MaritimeMissionsTab } from "@/components/maritime/MaritimeMissionsTab";
...
export default function MaritimeDemandesPage() {
  ...
  if (loading) return <TransportPageLoader message="Chargement expéditions..." vehicle="truck" />;
  ...
  return (
    <MaritimeDashboardLayout gpProfile={gpProfile} pendingCount={pendingOrders.length} activeOrdersCount={activeOrders.length}>
      <div className="px-3 py-3 space-y-3 pb-24">
        ...
        <Tabs defaultValue="orders" className="w-full">
          <TabsList className="w-full grid grid-cols-4 h-9">
            ...
            <TabsTrigger value="missions" className="text-[10px] data-[state=active]:text-primary">
              Missions
            </TabsTrigger>
            <TabsTrigger value="consolidation" className="text-[10px] data-[state=active]:text-primary">
              Groupage
            </TabsTrigger>
            <TabsTrigger value="routes" className="text-[10px] data-[state=active]:text-primary">
              Routes
            </TabsTrigger>
          </TabsList>
            ...
            {pendingOrders.length === 0 && activeOrders.length === 0 && (
              <Card className="border-dashed">
                <CardContent className="py-8 text-center">
                  <Ship className="w-10 h-10 text-muted-foreground/20 mx-auto mb-2" />
          ...
          <TabsContent value="missions" className="mt-3">
            <MaritimeMissionsTab gpId={gpProfile.id} />
          </TabsContent>
          ...
          <TabsContent value="consolidation" className="mt-3 space-y-3">
            <div className="bg-muted/50 rounded-xl p-3 border border-border">
              ...
              <p className="text-[10px] text-muted-foreground">
                Regroupez automatiquement les demandes clients sur un même corridor pour optimiser vos conteneurs.
              </p>
            </div>
            <MaritimeConsolidationCard 
              gpId={gpProfile.id} 
              onCreateDeparture={() => navigate("/maritime/publier")}
            />
          </TabsContent>
          ...
          <TabsContent value="routes" className="mt-3 space-y-3">
            <div className="bg-muted/50 rounded-xl p-3 border border-border">
              ...
              <p className="text-[10px] text-muted-foreground">
                Corridors à forte demande que vous ne couvrez pas encore. Ouvrez-les pour capter plus de fret.
              </p>
            </div>
            <MaritimeAutoRoutes 
              gpId={gpProfile.id} 
              onCreateDeparture={(origin, dest) => navigate("/maritime/publier")}
            />
          </TabsContent>
        </Tabs>
      </div>
