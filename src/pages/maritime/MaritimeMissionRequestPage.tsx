/**
 * MaritimeMissionRequestPage — Client creates a custom maritime mission
 * Simplified version of the booking flow for quick mission posting
 */
import { useState } from "react";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import {
  Ship, ArrowLeft, Check, Loader2, Package, MapPin,
...
export default function MaritimeMissionRequestPage() {
  ...
  const handleSubmit = async () => {
    if (!originCity || !destCity || !merchandiseType) {
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground">Les transitaires vont vous envoyer leurs propositions.</p>
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <header className="sticky top-0 z-50 bg-gradient-to-r from-primary to-primary/90 shadow-lg" style={{ paddingTop: 'calc(8px + var(--safe-top, 0px))' }}>
        <div className="px-3 py-2.5 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-primary-foreground font-bold text-sm">Demande maritime</h1>
          </div>
        </div>
      </header>
      ...
      <div className="px-4 py-4 max-w-lg mx-auto space-y-4">
        ...
        <Card>
          <CardContent className="p-3 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1"><Label className="text-[10px]">Ville d'origine *</Label><Input value={originCity} onChange={e => setOriginCity(e.target.value)} placeholder="Shenzhen" className="h-9 text-xs" /></div>
        ...
        <Card>
          <CardContent className="p-3 space-y-3">
            <div className="flex items-center gap-1.5 mb-1"><Package className="w-3.5 h-3.5 text-primary" /><span className="text-xs font-bold">Marchandise</span></div>
            ...
            <Textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Description détaillée de la marchandise..." rows={2} className="text-xs" />
            <div className="grid grid-cols-2 gap-3">
        ...
        <Card>
          <CardContent className="p-3 space-y-3">
            ...
            <div className="space-y-1"><Label className="text-[10px]">Budget proposé (FCFA)</Label><Input type="number" value={budget} onChange={e => setBudget(e.target.value)} placeholder="400000" className="h-9 text-xs" /></div>
            <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
            <span className="text-xs font-semibold">Assurance maritime</span>
            <Switch checked={insuranceRequired} onCheckedChange={setInsuranceRequired} />
          </div>
        </div>

        <Button className="w-full h-12" onClick={handleSubmit} disabled={submitting || !originCity || !destCity || !merchandiseType}>
